            ...
            if not api_key:
                ...
                with st.spinner("AI가 분석 중입니다... 🤖🔍"):
                    try:
                        ...
                        if web_info.strip():
                            prompt = f"상품 메인 사진과 검색정보야.\n[정보]\n{web_info}\n위 [정보]에 나오는 상품의 진짜 의미와 효과(예: 액막이, 재물운, 마음 안정 등)를 반드시 내용에 포함시켜 줘! 구구절절 긴 문장은 절대 피하고, 전체 글자수 30자 이내로 아주 짧고 간결한 쇼핑몰 카피라이팅을 딱 2줄로 작성해 줘. (예: 나쁜 기운은 막고, 곁에는 행운만. 당신을 지켜주는 붉은 수호석.)"
                        else:
                            prompt = "이 사진의 특징과 디테일을 파악해서, 구구절절 긴 문장은 절대 피하고 전체 글자수 30자 이내로 아주 짧고 간결한 쇼핑몰 카피라이팅을 딱 2줄로 작성해 줘. 시선을 확 끄는 핵심 단어 위주로 작성해. (예: 나쁜 기운은 막고, 곁에는 행운만. 당신을 지켜주는 붉은 수호석.)"
                    ...
                    if not f and not loaded_b64:
                        ...
                        with st.spinner("AI가 분석 중입니다..."):
                            try:
                                ...
                                if web_context:
                                    prompt = f"상품 사진과 관련 정보야.\n[정보]\n{web_context}\n위 [정보]에 나오는 상품의 상징과 효능(예: 액막이, 재물운 등)을 반드시 활용해 줘! 길고 지루한 설명은 다 빼고, 전체 글자수 30자 이내로 아주 짧고 임팩트 있는 카피라이팅을 딱 2줄로 작성해 줘. (예: 나쁜 기운은 막고, 곁에는 행운만. 당신을 지켜주는 붉은 수호석.)"
                                else:
                                    prompt = "이 상품 사진의 핵심 디테일과 감성을 살려서, 길고 지루한 설명은 다 빼고 전체 글자수 30자 이내로 아주 짧고 임팩트 있는 카피라이팅을 딱 2줄로 작성해 줘. 고객의 시선을 확 사로잡을 수 있도록 간결하게 써 줘. (예: 시선을 끄는 붉은 빛. 완벽한 존재감.)"
                                try:
                                    model = genai.GenerativeModel('gemini-2.5-flash')
                                    response = model.generate_content([prompt, img])
                                except Exception:
