...
def render_template(bg, style_idx, tmpl_color, tmpl_title, tmpl_sub_top, tmpl_sub_bottom, points):
    ...
    if style_idx == 0:
        ...
        banner_h = int(H * 0.16)
        banner_y = H - banner_h
        draw.rectangle([0, banner_y, W, H], fill=tmpl_color)
        ...
        for idx, (icon, title, desc) in enumerate(points):
            ...
            draw_text_with_fallback(draw, (icon_x, banner_y + banner_h*0.5), icon, f_icon, "white", "mm")
        ...
        draw_text_with_fallback(draw, (title_x, title_y), tmpl_sub_top, f_sub, "#222222")
        lines = tmpl_title.split("\n")
        curr_y = title_y + int(H * 0.05)
        for idx, line in enumerate(lines):
            try: bbox = draw.textbbox((title_x, curr_y), line, font=f_main)
            ...
            curr_y += int(H * 0.1)
        draw.text((title_x, curr_y + int(H * 0.02)), tmpl_sub_bottom, font=f_sub, fill="#333333")
        ...
        for idx, (icon, title, desc) in enumerate(points):
            cx = idx * col_w + (col_w // 2)
            draw_text_with_fallback(draw, (cx, banner_y), icon, f_icon, "white", "mm")
            draw_text_with_fallback(draw, (cx, banner_y + int(H*0.05)), title, f_p_title, tmpl_color, "mm")
        ...
        cy += int(H * 0.05)
        for line in lines:
            draw_text_with_fallback(draw, (cx, cy), line, f_main, "white", "mt")
            cy += int(H * 0.08)
            
        cy += int(H * 0.05)
        draw_text_with_fallback(draw, (cx, cy), tmpl_sub_bottom, f_sub, "#DDDDDD", "mm")
        
        cy += int(H * 0.15)
        f_icon = ImageFont.truetype(font_e, int(H * 0.05))
        f_p_title = ImageFont.truetype(font_b, int(H * 0.025))
        for (icon, title, desc) in points:
            draw_text_with_fallback(draw, (cx, cy), icon, f_icon, "white", "mm")
            draw_text_with_fallback(draw, (cx, cy + int(H*0.04)), title, f_p_title, "white", "mm")
            cy += int(H * 0.12)
        ...
        draw_text_with_fallback(draw, (tx, ty), inline_title, f_main, "#222222", "lt")
        ty += int(box_h * 0.25)
        draw_text_with_fallback(draw, (tx, ty), tmpl_sub_top, f_sub, tmpl_color, "lt")
        
        ty += int(box_h * 0.15)
        f_p_title = ImageFont.truetype(font_b, int(box_w * 0.07))
        for (icon, title, desc) in points:
            draw_text_with_fallback(draw, (tx, ty), f"{icon} {title}", f_p_title, "#444444", "lt")
            ty += int(box_h * 0.15)

    elif style_idx == 4:
        # 5. 투컬럼 스플릿 (화면 분할)
        split_x = int(W * 0.4)
        draw.rectangle([0, 0, split_x, H], fill=tmpl_color)
        ...
        cy += int(H * 0.05)
        for line in lines:
            draw_text_with_fallback(draw, (cx, cy), line, f_main, "white", "lt")
            cy += int(H * 0.08)
            
        cy += int(H * 0.05)
        draw_text_with_fallback(draw, (cx, cy), tmpl_sub_bottom, f_sub, "#DDDDDD", "lt")
        
        cy += int(H * 0.15)
        for (icon, title, desc) in points:
            draw_text_with_fallback(draw, (cx, cy), f"{icon} {title}", f_p_title, "white", "lt")
            draw_text_with_fallback(draw, (cx, cy + int(H*0.03)), desc, f_p_desc, "#CCCCCC", "lt")
            cy += int(H * 0.1)
        ...
        for idx, (icon, title, desc) in enumerate(points):
            ...
            left = cx - card_w//2
            draw.rounded_rectangle([left, card_y, left+card_w, card_y+card_h], radius=15, fill="white")
            
            draw_text_with_fallback(draw, (cx, card_y + card_h*0.2), icon, f_icon, "black", "mm")
            draw_text_with_fallback(draw, (cx, card_y + card_h*0.55), title, f_p_title, tmpl_color, "mm")
            draw_text_with_fallback(draw, (cx, card_y + card_h*0.8), desc, f_p_desc, "#555555", "mm")
        ...
        draw.rectangle([0, 0, W, header_h], fill=tmpl_color)
        draw.rectangle([0, H - footer_h, W, H], fill="#222222")
        ...
        for idx, (icon, title, desc) in enumerate(points):
            cx = idx * col_w + (col_w // 2)
            draw_text_with_fallback(draw, (cx, footer_y + footer_h*0.3), icon, f_icon, "white", "mm")
            draw_text_with_fallback(draw, (cx, footer_y + footer_h*0.7), title, f_p_title, tmpl_color, "mm")
        ...
        for i in range(grad_h):
            ...
            draw.line([(0, grad_y + i), (W, grad_y + i)], fill=(0, 0, 0, alpha))
        ...
        draw_text_with_fallback(draw, (title_x, title_y), tmpl_sub_top, f_sub, "#222222", "lt")
        lines = tmpl_title.split("\n")
        curr_y = title_y + int(H * 0.05)
        for line in lines:
            draw_text_with_fallback(draw, (title_x, curr_y), line, f_main, "white", "lt")
            curr_y += int(H * 0.1)
        ...
        for (icon, title, desc) in points:
            draw_text_with_fallback(draw, (title_x, curr_y), f"{icon} {title}", f_p_title, "#222222", "lt")
            draw_text_with_fallback(draw, (title_x, curr_y + int(H*0.04)), desc, f_p_desc, "white", "lt")
            curr_y += int(H * 0.1)

    elif style_idx == 9:
        # 10. 원형 배지(Badge) 강조형
        badge_r = int(W * 0.25)
        bx, by = W - badge_r - int(W*0.05), int(H*0.05)
        ...
        draw_text_with_fallback(draw, (cx, cy - int(badge_r*0.2)), short_title, f_main, "white", "mm")
        draw_text_with_fallback(draw, (cx, cy + int(badge_r*0.3)), tmpl_sub_top[:8], f_sub, "yellow", "mm")
        
        banner_h = int(H * 0.12)
        banner_y = H - banner_h
        draw.rectangle([0, banner_y, W, H], fill=(255,255,255,230))
